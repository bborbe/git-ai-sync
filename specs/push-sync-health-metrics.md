...
- [ ] Unit tests cover: heartbeat pushed every watch cycle, last-success pushed only on success, push failure does not abort the sync loop, unconfigured pushgateway disables the feature
...
- [ ] Negative: with no `GIT_AI_SYNC_PUSHGATEWAY_URL` set, `git-ai-sync watch` runs with no push attempt and `make test` passes unchanged
- [ ] Log rotation: `git-ai-sync` log output is bounded — after a multi-day run the log file is ≤ a documented size (rotated, not appended unboundedly)
...
- `grep -n 'git_ai_sync_last_success_timestamp' src/git_ai_sync/metrics.py` — the metric names exist in the new module
...
4. When no pushgateway URL is configured, metric pushing is fully disabled — zero network calls, no behavior change to the sync loop
